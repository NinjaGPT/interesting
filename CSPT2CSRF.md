...
每个安全研究人员都应该了解什么是路径遍历漏洞。这种漏洞使攻击者能够使用像 ../../../../ 这样的负载读取目标目录之外的数据。与读取服务器文件的服务器端路径遍历攻击不同，客户端路径遍历攻击则专注于利用这种弱点向意图之外的 API 端点发出请求。尽管这类漏洞在服务器端非常流行，但客户端路径遍历的案例却很少被广泛公开。我们发现的第一个参考资料是 Philippe Harewood 在 Facebook 漏洞赏金计划中报告的一个漏洞。从那时起，我们只发现了少数有关客户端路径遍历的参考资料：
```
...
攻击者可以迫使受害者执行这个意外的请求。这就是客户端路径遍历（CSPT）的起点。客户端路径遍历可以分为两部分。源是 CSPT 的触发点，而汇是可以通过这个 CSPT 到达的可利用端点。
...
➧ 基于 DOM 的：page#id=XXXXXXXXX 或任何从 DOM 可访问的数据（例如路径）
➧ 存储型：从数据库读取的输入
```
...
```
➧ 主机：如果源正在访问 api.doyensec.com 后端，你将无法定位到另一个主机。
➧ HTTP 方法：通过 CSPT 你无法改变请求的 HTTP 方法。然而，完全有可能找到使用 GET、POST、PATCH、PUT 或 DELETE 方法的汇。
➧ 头部信息：源可以添加后端所需的一些附加头部信息（例如，CSRF 令牌和身份验证令牌）。
➧ 请求体内容：源可能在请求中包含请求体内容。CSPT 无法控制请求体内容，除非请求体内容基于其他用户输入。
```
汇是一个可到达的端点，具有相同的限制条件。它将定义攻击者可以利用关联源执行的操作。确实，在同一个应用程序中，有可能找到另一个 CSPT 源，例如，具有不同的 HTTP 方法或不同的请求体内容，因此会产生不同的影响。
...
➧ 由于我们可以控制请求路径，在大多数情况下，我们可以控制源发送的查询参数。在这种情况下，我们可以添加会被后端读取的参数。注意：使用这些查询参数，可能覆盖请求体内容中定义的参数。
```
一旦识别出所有汇的限制条件，所有满足这些要求的可到达端点都可以被列出，并且可以定义 CSPT 的影响。可以通过手动从文档中、从 JavaScript 代码中或使用 Burp Suite 的 Bambda 功能来列出所有的汇。
...
攻击者可以迫使受害者在选择的端点上触发 HTTP 请求。即使有一些限制（例如主机、方法、请求体、头部），也可能被利用。在这篇白皮书中，我们将重点介绍使用客户端路径遍历（CSPT）来触发 CSRF 攻击（CSPT2CSRF）。

CSPT 重新路由合法的 HTTP 请求，前端可能会添加执行 API 调用所需的令牌（例如，身份验证令牌、CSRF 令牌）。因此，它可以用于绕过现有的防护措施来防止 CSRF 攻击。如果攻击者能够找到有影响的汇，就可以使用 CSPT 作为攻击向量，在现代浏览器上执行 CSRF 攻击。因此，我们将这种新技术命名为 CSPT2CSRF。
...
为了确定是否可以利用此漏洞并评估其影响，我们需要识别可到达的汇。在这个例子中，所有潜在的汇必须符合以下限制：
```
➧ 主机：www.doyensec.com
➧ HTTP 方法：POST
➧ 头部信息：Authorization，CSRF-Token
➧ 请求体内容：{}
```
因此，攻击者可以使用此 CSPT 对兼容的汇执行 CSRF 攻击（CSPT2CSRF）。根据触发源和可到达汇的复杂性，发现的严重性有所不同。
...
➧ CSRF 保护措施仅基于 SameSite cookies 和 CSRF 令牌，因此可以通过 CSPT2CSRF 绕过这些保护措施。
```
...
```
后端在接受额外的 JSON 参数方面比较宽松。它没有实施严格的 JSON 架构验证。即使一个端点不需要 url 或 downloadId JSON 参数，请求仍然会被执行。后端也不允许将 JSON 参数更改为 GET 参数。
...
```
`telemetryEventForPlaybookRun` 将 `playbookRunI`D 值连接到路径并执行一个 POST 请求：
```
...
await doFetchWithoutResponse(`${apiUrl}/telemetry/run/${playbookRunID}`, { method: 'POST',
...
要利用此漏洞，必须在 telem_run_id 参数中设置有效负载。使用这个源，攻击者能够在选定的端点上触发 CSPT2CSRF。
...
注意：插件默认不会启用，但存在另一个兼容的 sink 来启用插件（`POST http://your-mattermost-url.com/api/v4/plugins/{plugin_id}/enable`）。

注意 2：`install_from_url` 端点在云实例上不可用，并且在本地部署中可能默认未启用。

以下是其他有影响力的 sink 的非详尽列表：
```
...
要利用此漏洞，有效载荷必须设置在 `channel_id` 值中，但 Mattermost 中的 ID 在创建时是随机生成的，无法修改。
然而，攻击者可以使用 `/api/v4/files` 端点上传一个恶意的 JSON 文件。请求时，该文件将以 `application/json` 形式提供。

恶意的 JSON 有效载荷必须格式化为类似于频道响应数据（`/api/v4/teams/name/<team_name>/channels/name/<channel_name>`），其中包含指向目标 CSRF 端点（例如，`../caches/invalidate`?）的恶意 id：
...
这可以通过在 Web 应用程序中构造一个 CSPT 有效载荷 URL 来实现，例如 `/<team_name>/channels/%2e%2e%2f%2e%2e%2f%2e%2e%2f%2e%2e%2ffiles%2f<file_id>`，如果在 Mattermost 内部共享，或通过双重 URL 编码，如果攻击来自外部网站。
...
5.前端尝试通过向`/api/v4/channels/../caches/invalidate?/members`发出 POST 请求来添加用户，这等同于在`/api/v4/caches/invalidate`
上发出 POST 请求。这确认了 CSRF 有效负载已被执行。

使用这个 source，攻击者能够在选定的端点（例如，`/api/v4/caches/invalidate`）上触发一个 one-click CSRF。

作为攻击者，使用文件小工具，可以链式触发两个 CSPT2CSRF：
...
```
➧ 后端必须强制执行 JSON 架构验证。确实，对接受的 JSON 参数严格要求可以大大减少兼容的 sinks，从而减少此类攻击的影响。
...
```
➧ CSPT 标签将读取代理历史记录，以查找 URL 路径中查询参数的反射。
...
工具的局限性：
```
➧ 除非你使用 canary 令牌作为输入数据，否则不会识别基于 DOM 或存储的 sources。
➧ 一些前端实现了客户端路由。此类路由不会发送 HTTP 请求到 Burp，因此除非你使用 canary 令牌，否则不会被我们的扩展捕获。
```
